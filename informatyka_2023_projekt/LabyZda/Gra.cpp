...
void Gra::initGIU()
{
	...
	this->Koniec.setPosition(this->window->getSize().x / 2.f - this->Koniec.getGlobalBounds().width / 2, this->window->getSize().y / 2.f - this->Koniec.getGlobalBounds().height / 2);
...
void Gra::updatePrzeciwnik()
{
	this->spawnTimer += 0.5f;
	if (this->spawnTimer >= this->spawnTimerMax)
	{
		this->przeciwnicy.push_back(new Przeciwnik(rand() % this->window->getSize().x - 20.f, -150.f));
		this->spawnTimer = 0.f;
	}
...
void Gra::renderGUI()
{
	...
	this->window->draw(this->graczHPB);
	this->window->draw(this->graczHP);
}
